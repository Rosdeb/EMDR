...
class _SessionfourpageState extends State<Sessionfourpage> {
  ...
  String _sessionType = '';
  String? _emdrSessionId;
  int? _vocRating;
  int? _pfsRating;
  String? _primaryEmotion;
  String? _additionalEmotions;
  String? _emdrBodyLocation;
  String? _addictionAspect;
  ...
  bool get _isAddictionFlow =>
      _responses.isNotEmpty && _responses[0].toString().contains('Addiction');

  // ── Starting choice ────────────────────────────────────────

  void _handleStartingChoice(String choice) async {
    String label;
    switch (choice) {
      case 'memory':
    ...
    final started = await _startEmdrSession(choice);
    if (!started) {
      return;
    }
  ...
  Future<void> _submitTextAnswer() async {
    ...
    if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains('freeze frame') &&
        _emdrSessionId != null) {
      final targetDescription = _responses.length >= 2
          ? _responses[1].toString()
          : '';
      await _saveTarget(targetDescription, text);
    }

    if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains(
          'what emotions are you noticing',
        )) {
      _primaryEmotion = text;
    } else if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains('are there other emotions')) {
      _additionalEmotions = text;
    } else if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains('where do you feel')) {
      _emdrBodyLocation = text;
      await _saveEmotionsIfReady();
    } else if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains(
          'what aspect of this addictive behavior',
        )) {
      _addictionAspect = text;
    } else if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains(
          'what is the specific positive feeling',
        )) {
      _positiveFeeling = text;
    } else if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains('are there any thoughts')) {
      _associatedThoughts = text;
    } else if (currentQuestion != null &&
        currentQuestion.toLowerCase().contains(
          'where does this positive feeling sit in your body',
        )) {
      _emdrBodyLocation = text;
        ...
        currentQuestion.toLowerCase().contains('what color, shape, or image')) {
      _visualization = text;
      await _saveAddictionIfReady();
    }
  ...
  Future<void> _finishSession() async {
    ...
    if (_emdrSessionId != null) {
      await _completeEmdrSession();
    }
    _saveSummary();
    _scrollToBottom();
  }
  ...
  Future<void> _handleBack() async {
    if (_emdrSessionId != null && _uiState != _UIState.summary) {
      final token = Get.find<AuthController>().token;
      if (token != null) {
        // Fire and forget abandon call
        EmdrSessionService.abandonSession(token, _emdrSessionId!);
      }
    }
    Get.back();
  }
  ...
  Future<bool> _startEmdrSession(String sessionType) async {
    final token = Get.find<AuthController>().token;
    if (token == null) {
      _showError('Please log in to start an EMDR session.');
      return false;
    }

    final result = await EmdrSessionService.startSession(token, sessionType);
    if (result['success'] == true && result['data'] != null) {
      _emdrSessionId = result['data']['_id'] as String?;
      return true;
    }

    _showError(
      result['message']?.toString() ?? 'Unable to start EMDR session.',
    );
    return false;
  }

  Future<void> _saveTarget(String targetDescription, String freezeFrame) async {
    if (_emdrSessionId == null) return;
    final token = Get.find<AuthController>().token;
    if (token == null) return;

    final result = await EmdrSessionService.saveTargetWithFiles(
      token,
      _emdrSessionId!,
      targetDescription: targetDescription,
      freezeFrame: freezeFrame,
      targetFile: _targetFile,
      freezeFrameFile: _freezeFrameFile,
    );
    if (result['success'] != true) {
      _showError(
        result['message']?.toString() ?? 'Failed to save target details.',
      );
    }
  }

  Future<void> _saveBeliefsIfReady() async {
    if (_emdrSessionId == null || _beliefPairs.isEmpty || _vocRating == null) {
      return;
    }
    final token = Get.find<AuthController>().token;
    if (token == null) return;
    ...
    if (result['success'] != true) {
      _showError(result['message']?.toString() ?? 'Failed to save beliefs.');
    }
  }

  Future<void> _saveEmotionsIfReady() async {
    if (_emdrSessionId == null ||
        _primaryEmotion == null ||
        _additionalEmotions == null ||
        _emdrBodyLocation == null) {
      return;
    }
    ...
    final result = await EmdrSessionService.saveEmotions(
      token,
      _emdrSessionId!,
    ...
    if (result['success'] != true) {
      _showError(result['message']?.toString() ?? 'Failed to save emotions.');
    }
  }

  Future<void> _saveAddictionIfReady() async {
    if (_emdrSessionId == null ||
        _addictionAspect == null ||
        _positiveFeeling == null ||
        _pfsRating == null ||
        _associatedThoughts == null ||
        _emdrBodyLocation == null ||
        _visualization == null) {
      return;
    }
    ...
    final body = {
      'aspect': _addictionAspect,
      'positiveFeeling': _positiveFeeling,
      'pfsRating': _pfsRating,
      'associatedThoughts': _associatedThoughts,
      'bodyLocation': _emdrBodyLocation,
      'visualization': _visualization,
    };

    final result = await EmdrSessionService.saveAddiction(
      token,
      _emdrSessionId!,
      body,
    );
    if (result['success'] != true) {
      _showError(
        result['message']?.toString() ?? 'Failed to save addiction details.',
      );
    }
  }

  Future<void> _saveSud(int value) async {
    if (_emdrSessionId == null) return;
    final token = Get.find<AuthController>().token;
    if (token == null) return;

    final result = await EmdrSessionService.saveSud(
      token,
      _emdrSessionId!,
      value,
    );
    if (result['success'] != true) {
      _showError(result['message']?.toString() ?? 'Failed to save SUD rating.');
    }
  }

  Future<void> _completeEmdrSession() async {
    if (_emdrSessionId == null) return;
    final token = Get.find<AuthController>().token;
    if (token == null) return;

    final result = await EmdrSessionService.completeSession(
      token,
      _emdrSessionId!,
    );
    if (result['success'] != true) {
  ...
  Future<void> _pickAttachment() async {
    ...
    // Determine whether we are attaching for target or freeze-frame
    // based on how many text answers have been given so far
    setState(() {
  ...
  Future<void> _submitPositiveBelief(String belief) async {
    _beliefPairs.add(
      BeliefPair(
    ...
    if (_negativeBeliefIndex < _selectedNegativeBeliefs.length) {
      ...
      _currentStep++;
      await _saveBeliefsIfReady();
      setState(() => _uiState = _UIState.chat);
      Future.delayed(const Duration(milliseconds: 700), _askNext);
    }
  }

  Future<void> _submitRating(String type, int value) async {
    String label;
    if (type == 'voc') {
    ...
    if (type == 'sud' && _emdrSessionId != null) {
      await _saveSud(value);
    }
    if (type == 'voc') {
      await _saveBeliefsIfReady();
    }
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: (_targetFile != null || _freezeFrameFile != null)
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                color: (option['color'] as Color).withOpacity(0.6),
                width: 2,
              ),
            ),
            child: Column(
...
class _BeliefSelectionWidget extends StatefulWidget {
  ...
  @override
  State<_BeliefSelectionWidget> createState() => _BeliefSelectionWidgetState();
}
                  ...
                  (i) {
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          color: isSel ? const Color(0xFF537E5D) : Colors.white,
                          border: Border.all(
                            color: isSel
                                ? const Color(0xFF537E5D)
                                : Colors.black12,
