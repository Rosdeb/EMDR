...
const _kName = 'Trauma';
const _kTrackerNum = '12';
const _kMaxScore = 40;

const _kItems = [
    TrackerItem(text: 'The event has come back into my mind when I did not want it to.', reverse: false),
    TrackerItem(text: 'Sleep has been disturbed by dreams connected to what happened.', reverse: false),
    TrackerItem(text: 'There have been moments when it has felt as though it were happening again.', reverse: false),
    TrackerItem(text: 'Reminders of it have stirred up strong distress in me.', reverse: false),
    TrackerItem(text: 'I have steered away from thinking about it, or talking about it.', reverse: false),
    TrackerItem(text: 'Reminders of what happened have felt manageable, more memory than alarm.', reverse: true),
    TrackerItem(text: 'I have kept clear of people, places, or situations that bring it up.', reverse: false),
    TrackerItem(text: 'My body has stayed alert in a way that does not switch off — scanning, braced, easily startled.', reverse: false),
    TrackerItem(text: 'Closeness has felt harder, and warm feelings less available.', reverse: false),
    TrackerItem(text: 'Something about how I see myself, others, or the world has been damaged or shaken.', reverse: false),
];

const _kBands = [
    SeverityBand(max: 9, label: 'Minimal', description: 'Few trauma-related symptoms reported over the past week. This may reflect distance from the event, processing already done, or resilience.'),
    SeverityBand(max: 17, label: 'Mild', description: 'Some trauma-related symptoms present. EMDR is well-suited to working with these. Track how these change over time as you progress through the programme.'),
    SeverityBand(max: 25, label: 'Moderate', description: 'A meaningful pattern of trauma-related symptoms over the past week. The programme is designed for exactly this kind of work. Pace yourself, follow the programme structure carefully, and track how this score shifts over the coming weeks.'),
    SeverityBand(max: 32, label: 'Marked', description: 'Substantial trauma-related symptoms with likely impact on functioning and wellbeing. Self-directed EMDR can be helpful at this level but it is important to pace carefully, use the resources and grounding tools the programme provides, and consider whether working with a qualified EMDR therapist might be a better fit for your current presentation.'),
    SeverityBand(max: 40, label: 'Severe', description: 'A high level of trauma-related symptoms with significant impact across multiple areas of life. At this level of distress, working through trauma alone with a self-directed programme is not recommended. Please consider working with a qualified EMDR therapist instead — you can find one through the EMDR UK & Ireland Association directory (emdrassociation.org.uk).'),
];
...
class TraumaScreen extends StatefulWidget {
  const TraumaScreen({super.key});

  @override
  State<TraumaScreen> createState() => _TraumaScreenState();
}
