...
const _kName = 'Specific Phobia';
const _kTrackerNum = '06';
const _kMaxScore = 40;

const _kItems = [
    TrackerItem(text: 'The fear has gripped me when I encountered it, or pictured encountering it.', reverse: false),
    TrackerItem(text: 'My body has reacted strongly when I was near it (heart, breath, dizziness, shaking).', reverse: false),
    TrackerItem(text: 'I have arranged my life around not coming across it.', reverse: false),
    TrackerItem(text: 'The fear has felt out of step with the actual risk.', reverse: false),
    TrackerItem(text: 'I have changed routes, plans, or activities to keep clear of it.', reverse: false),
    TrackerItem(text: 'When I have come across it, I have stayed in contact with myself.', reverse: true),
    TrackerItem(text: 'The dread has reached beyond the moments of actual exposure.', reverse: false),
    TrackerItem(text: 'I have leaned on others to help me dodge or get through encounters.', reverse: false),
    TrackerItem(text: 'The fear has narrowed what I can do at work, in travel, in leisure, or with people.', reverse: false),
    TrackerItem(text: 'The size of this fear has frustrated or embarrassed me.', reverse: false),
];

const _kBands = [
    SeverityBand(max: 9, label: 'Minimal', description: 'The fear is reported as having had little impact across the past week. This may reflect successful avoidance, infrequent exposure, or a fear that is contained.'),
    SeverityBand(max: 17, label: 'Mild', description: 'Some impact from the fear, mostly manageable. Worth noticing what avoidance you are doing — the cost is sometimes invisible.'),
    SeverityBand(max: 25, label: 'Moderate', description: 'A meaningful impact of this fear over the past week. EMDR can be effective for specific phobias, particularly when there is a clear original trigger. Track this score over time to notice changes.'),
    SeverityBand(max: 32, label: 'Marked', description: 'Substantial impact from this fear with likely effect on activities, choices, and wellbeing. Continue with the programme, and consider whether additional support might be useful.'),
    SeverityBand(max: 40, label: 'Severe', description: 'A high level of impact from this fear across multiple areas of life. At this level, additional support is recommended alongside the programme — please consider speaking to a qualified mental health professional.'),
];
...
class SpecificPhobiaScreen extends StatefulWidget {
  const SpecificPhobiaScreen({super.key});

  @override
  State<SpecificPhobiaScreen> createState() => _SpecificPhobiaScreenState();
}

class _SpecificPhobiaScreenState extends State<SpecificPhobiaScreen> {
  ...
  void _submit() async {
    ...
    _stemValue = _stemController.text.trim();
    int total = 0;
    final scores = <int>[];
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFFC9D6D6)),
                ),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Before you begin',
                      style: TextStyle(fontFamily: 'Fraunces', fontSize: 16, fontWeight: FontWeight.w500, color: Color(0xFF1A1814))),
                    const SizedBox(height: 8),
                    const Text('Name the specific focus for this tracker.',
                      style: TextStyle(fontSize: 14, color: Color(0xFF4A4540), height: 1.55)),
                    const SizedBox(height: 16),
                    TextField(
                      controller: _stemController,
                      decoration: const InputDecoration(
                        hintText: 'Type here...',
                        hintStyle: TextStyle(fontFamily: 'Fraunces', color: Color(0xFF8A8278), fontStyle: FontStyle.italic),
                        border: OutlineInputBorder(borderRadius: BorderRadius.zero, borderSide: BorderSide(color: Color(0xFFDDD5C5))),
                        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.zero, borderSide: BorderSide(color: Color(0xFF4A7373))),
                        filled: true, fillColor: Color(0xFFF5F1EA),
                        contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 14),
                      ),
                      style: const TextStyle(fontFamily: 'Fraunces', fontSize: 17, color: Color(0xFF1A1814)),
                    ),
                  ],
                ),
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
            ...
            if (_stemValue.isNotEmpty)
              Padding(
                padding: const EdgeInsets.only(bottom: 16),
                child: Text(
                  'In relation to $_stemValue',
                  style: const TextStyle(fontFamily: 'Fraunces', fontSize: 15, fontStyle: FontStyle.italic, color: Color(0xFF4A4540)),
                ),
              ),
          Row(
